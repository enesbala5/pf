<script lang="ts">
	...
	import { sineInOut, sineIn, quartInOut } from 'svelte/easing';
...
<!-- infobar -->
<div class="bg-neutral-200 transition-colors delay-150 ease-in-out dark:bg-darkgray" id="wrapper">
...
<!-- hero -->
<IntersectionObserver element={hero} bind:intersecting={intersectingHero} threshold={0.7}>
	<div
		bind:this={hero}
	...
	>
		<div
			...
			class="
			{frontendHover ? 'opacity-100' : 'opacity-0'}
			z-10 col-span-10 col-start-1 row-start-2 row-end-6 mb-12 flex w-full items-center justify-center overflow-hidden px-4 transition-opacity delay-75 ease-out lg:col-span-7 lg:col-start-4  lg:mb-10 lg:mt-10 lg:px-0 "
		>
			<Keyboard red />
		</div>
		<div
			bind:this={uiUxImage}
			class="
			{uiuxHover ? 'opacity-100' : 'opacity-0'}
			z-10 col-span-3 col-start-7 row-start-1 row-end-6 mt-24 w-full items-center justify-center overflow-hidden  transition-opacity delay-75 ease-out "
		>
			<Palette red />
		</div>
		<div
		...
		>
			<h1 class="mb-4 text-7xl font-medium leading-[3.5rem] ease-in-out md:text-7xl lg:text-8xl">
				Product Designer
			</h1>
			<div class="mt-4 flex w-full items-end justify-between">
				<div>
					<h2
						...
						class="text-3xl font-medium transition-all delay-75 hover:opacity-90 lg:text-3xl xl:text-4xl
							{frontendHover && innerWidth > 1024 ? 'opacity-90' : 'opacity-40'}"
					>
						Frontend Developer
					</h2>
					<h2
						...
						class=" text-3xl font-medium transition-all delay-75 hover:opacity-90 lg:text-3xl xl:text-4xl
						{uiuxHover && innerWidth > 1024 ? 'opacity-90' : 'opacity-40'}"
					>
						UI& UX Designer
					</h2>
				</div>
	...
	<div class="my-64 hidden lg:block" bind:this={element}>
		{#if browser}
			<AnimateSharedLayout children>
				<Motion let:motion transition={{ duration: 1 }} layout>
					<div
						use:motion
						class="relative mx-auto mt-44 mb-12 grid w-10/12 grid-cols-10 grid-rows-1 gap-2 text-xs"
					>
						<Motion let:motion={m1} transition={{ duration: 0.5 }} layout>
							<div
								use:m1
						...
						<Motion let:motion={m2} transition={{ duration: 0.5 }} layout>
							<div
								use:m2
...
>
	<!-- titlebar -->
	<!-- end mobile only -->
	<div class="mb-4 flex items-center justify-between pb-4 lg:hidden">
		<div class="">
	...
	<div class=" hidden lg:col-span-3 lg:col-start-3 lg:block">
		<div class="flex items-center space-x-4 opacity-70">
			<a
				href={navigation.work}
				class="text-base opacity-70"
	...
	{#each projects as project, i}
		{#if i < 3}
			<div class="col-span-8 flex items-center space-x-2 odd:col-start-2 even:col-start-3">
				<ProjectCard
...
<CustomRedirect link={navigation.about}>About me</CustomRedirect>
<!-- <ContactCta /> -->
<Footer />
