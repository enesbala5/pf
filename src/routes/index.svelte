...
<div class="bg-darkgray">
	...
	>
		...
		<div class="col-span-2 flex items-center opacity-70">Name Surn</div>
		<div
...
>
	<div class="col-span-2">
		...
		<p class="opacity-20">Hover for more</p>
	</div>
	<div class="col-span-2 col-start-3">
		<div
			...
			on:mouseleave={notHovering}
		>
			<p class="peer opacity-70 transition-all delay-75 hover:opacity-100">
				Frontend Development
			</p>
			<div
				class="peer mt-0.5 h-0.5 w-1 bg-white opacity-0 transition-all delay-75 peer-hover:w-full peer-hover:opacity-100"
			/>
		</div>
	</div>
	...
	>
		{#each designSkills as designSkill}
			<div
				class="group flex w-full items-center justify-between"
				on:mouseenter={() => hoverSkill(designSkill.icon)}
			>
				<div class="h-[1px] w-full bg-lightgray" />
				<p
					class="ml-5  whitespace-nowrap text-right text-sm opacity-70 group-hover:opacity-100"
	...
	>
		{#each otherSkills as otherSkill}
			<div
				class="group flex w-full items-center justify-between"
				on:mouseenter={() => hoverSkill(otherSkill.icon)}
			>
				<div class="h-[1px] w-full bg-lightgray" />
				<p
					class="ml-5  whitespace-nowrap text-right text-sm opacity-70 group-hover:opacity-100"
	...
	<div
		class="peer absolute col-span-2 col-start-6 flex aspect-square w-full items-center justify-center bg-darkgray opacity-0 transition-all delay-75 peer-hover:opacity-100"
	>
				...
				<Motion let:motion={m1} transition={{ duration: 0.5 }} layout>
					<div
						...
						class="col-span-6  row-start-1 row-end-2 h-[70vh] {hovering
							...
							: 'col-start-3'}"
					>
						<div class="h-full w-full bg-darkgray" />
						<p class="mt-2 font-mono opacity-50">Project FirstLast™</p>
					</div>
				</Motion>
				<Motion let:motion={m2} transition={{ duration: 0.5 }} layout>
					<div
						...
						class="z-20  col-span-4 row-start-1 row-end-2 flex h-[70vh] flex-col justify-center {hovering
							...
							: 'col-start-7'}"
					>
						<div class="h-[40vh] w-full bg-lightgray " />
						<p class="mt-2 font-mono opacity-50">2018 Name Surn</p>
					</div>
				</Motion>
			</div>
		</Motion>
...
>
	...
	<div class=" col-span-2 col-start-3">
		<div
			class="flex items-center space-x-4 opacity-70"
			on:mouseenter={hoveredOverLink}
			on:mouseleave={notHovering}
		>
	...
	<div class="col-span-6 col-start-5 flex justify-end"
		...
		on:mouseleave={notHovering}
	>
		<a href={navigation.work} class="text-base opacity-70">View all</a>
	</div>
	<!-- end titlebar -->
...
<!-- logo panel -->
<div class="my-12 mt-44 bg-darkgray">
